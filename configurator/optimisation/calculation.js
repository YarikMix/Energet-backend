...
const {Common_Sources, Common_DSources, Common_Storages, OPTIMISATION_TARGETS, OPTIMISATION_LOAD, CONSUMPTION_GET, SOLPROD_GET, VARSSS} = require("../utils/consts")
...
// почасовки от энергосбыт компании
        ...
        const ans = await this.optimisation(await params, horison, GHI, temperature, pressure, Wind_Speed)
    ...
    optimisation (params, horison, GHI, temperature, pressure, Wind_Speed) {
        ...
        let consumption = Optim.consumption(params.load.name,params.load.value,params.coords, OPTIMISATION_LOAD);
        consumption = consumption.splice(Math.round(params.coords[1]/15)).concat(consumption);
        ...
        let refining = 0;
        let Vars = Optim.vars(Common_Sources,  params.EnSource, Common_DSources,  params.EnDSource, Common_Storages,  params.EnStorage, N_steps, step, mid_load, refining);
...
// console.log(Varsn)
                for (let i = 0; i < Varsn.length; i++) {
                        let SolarProd = Solar.production(params.coords, horison, params.Additions.shading, GHI, temperature, Varsn[i][0][0], Varsn[i][1]);
...
// надо будет вернуть расчет солнца
                        ...
                        let NPV = Optim.NPV(Energy_AfterDGS[0],  CapEx,  OpEx, params.ElCost,  consumption, 30, 0.1);
                        NPV_global =  NPV;

                        let LCOE = Optim.LCOE(Energy_AfterDGS[0],  CapEx,  OpEx,  consumption, 30, 0.1);
...
console.log("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%")
                        ...
                        let Economy = Optim.Economy(Energy_AfterDGS[0], params.ElCost,  consumption, 30, 0.1);

                        Res_Array = Optim.getResArr(i, Res_Array, params.OptTarget, OPTIMISATION_TARGETS, Varsn, CapEx, OpEx, RPS, NPV, Economy, LCOE, target.value, OptVal_prev, Energy_AfterDGS[1], Energy[2], Energy[1], SolarProd, WindProd);
// return Res_Array
...
// console.log(Varsn)
                ...
                if ((Res_Array[0].Opt_Vars == "") || (Res_Array[0].Condition)) {
                        ...
                        step = step*2;
                        refining = 0;
                        Vars = Optim.vars(Common_Sources,  params.EnSource, Common_DSources,  params.EnDSource, Common_Storages,  params.EnStorage, N_steps, step, mid_load, refining);
                        ...
                        step = step*0.5;
                        refining = 1;
                        Vars = Optim.vars(Common_Sources,  params.EnSource, Common_DSources,  params.EnDSource, Common_Storages,  params.EnStorage, N_steps, step, 1, refining);
                        Varsn = [];
                        for (let i = 0; i < Vars.length; i++) {
